import { HealthCheckService, HealthCheckResult, ServiceHealth } from '../../../api/services/health-check.service';
...
jest.mock('../../../api/services/error-handler.service', () => ({
    ErrorHandlerService: {
        withTimeout: jest.fn().mockImplementation(async (fn, name, timeout) => {
            return await fn();
        })
    }
}));
